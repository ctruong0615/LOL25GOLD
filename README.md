...
|---:|:---------|:-------|-----------:|-----------:|-----------:|-----------:|---------------:|---------------:|---------------:|---------------:|--------------:|------------------:|----------------------:|----------:|--------------:|---------:|
|  0 | LFL2     | Blue   |       3058 |       4531 |       6473 |       9244 |           -336 |           -841 |           -539 |            224 |           nan |               nan |                   nan |       nan |           nan |        0 |
|  1 | LFL2     | Blue   |       2977 |       4461 |       5668 |       7040 |           -474 |           -828 |          -1689 |          -2363 |           nan |               nan |                   nan |       nan |           nan |        0 |
|  2 | LFL2     | Blue   |       3020 |       4581 |       6332 |       8488 |           -405 |          -1085 |          -1658 |          -1552 |           nan |               nan |                   nan |       nan |           nan |        0 |
|  3 | LFL2     | Blue   |       3097 |       4687 |       6782 |       9097 |           -422 |           -911 |          -1707 |          -2613 |           nan |               nan |                   nan |       nan |           nan |        0 |
|  4 | LFL2     | Blue   |       2114 |       3238 |       4220 |       5357 |            -85 |           -172 |           -310 |           -662 |           nan |               nan |                   nan |       nan |           nan |        0 |
...
For our final model, we used all the gold columns, that is, `goldat10`, `goldat15`, `goldat20`, `goldat25`, `opp_goldat10`, `opp_goldat15`, `opp_goldat20`, and `opp_goldat25`. To use these all we combined them into one feature, by first computing the differences at 10,15,20, and 25 minutes. Then, we applied a standard scale because, as the game goes on, gold differences often get larger. Finally, we summed them all together. I think this new feature we created is helpful because even if a team is now behind on gold, if they were previously ahead, it could help them in the future because they were able to get objectives, turrets, or inhibitors. Therefore, we still care about this information from the past. I also think that it is helpful that we are now using gold differences because it is possible for both teams in a game to have a lot of gold, but this can not mean they both have an above 50% chance of winning. The other features we used are `elementaldrakes` and `opp_elementaldrakes`. To use these features, we paired them together for each row. That is, if `elementaldrakes` is 2 and `opp_elementaldrakes` is 4, the value would be (2,4). Then, we used one-hot encoding based on this new column. Similar to before, this feature is helpful because it lets us compare the ally teams' drakes to the enemy teams. Without this knowledge, we may think 3 drakes are good, but if the enemy has 4, this team is obviously behind. 

Again, we used a random forest and tuned the following hyperparameters: max_depth, min_samples_leaf, and max_features. We selected our hyperparameters by using gridsearchcv, and the best ones were a max_depth of 10, min_samples_leaf of 10, and None for max_features. We achieved a test accuracy of 85.6%. This is a large improvement over our baseline model (about 22%). However, this model does use information from a later game state, but I think this tradeoff is justifiable given the much better accuracy. 

We additionally performed permutation testing to see the importance of drakes/opp_drakes and gold/opp_gold. This time, we actually found that drakes were more important than gold, getting an 80% accuracy, where gold alone got a 70%. This tells us that drakes are more important later than gold, though gold is more important early. This makes sense because an early gold advantage helps teams get future objectives, whereas an early objective does not help nearly as much.
